...
func main() {
...

	{
		...
		foo := new(bistack_models.Foo).Stage(stack1Instance1)
		updateNameBeat(1*time.Second, foo, "Stack 1 Instance 1", stack1Instance1)
	}

	{
		stack1Instance2 := bistack_stack.NewStack(r, bistack_models.Bistack_Stack1_Instance2.ToString(),
			"", "", "", *embeddedDiagrams, false).Stage
		foo := new(bistack_models.Foo).Stage(stack1Instance2)

		updateNameBeat(2*time.Second, foo, "Stack 1 Instance 2", stack1Instance2)
	}

	{
		...
		bar := new(otherstack_models.Bar).Stage(otherstack_instance1)

		updateNameBeat(3*time.Second, bar, "Stack 2 Instance 1", otherstack_instance1)
	}

	{
		otherstack_instance2 := otherstack_stack.NewStack(r, otherstack_models.Otherstack_Instance2.ToString(),
			"", "", "", *embeddedDiagrams, true).Stage
		// get first element
		bar := new(otherstack_models.Bar).Stage(otherstack_instance2)

		updateNameBeat(4*time.Second, bar, "Stack 2 Instance 2", otherstack_instance2)
	}
