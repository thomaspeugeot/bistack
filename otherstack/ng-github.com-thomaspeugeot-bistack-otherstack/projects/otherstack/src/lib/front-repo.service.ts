...
export const StackType = "github.com/thomaspeugeot/bistack/otherstack/go/models"
...
export class FrontRepoService {
	...
	public connectToWebSocket(GONG__StackPath: string): Observable<FrontRepo> {
		...
		let basePath = 'ws://localhost:8080/api/github.com/thomaspeugeot/bistack/otherstack/go/v1/ws/stage'
		...
		return new Observable(observer => {
			this.socket!.onmessage = event => {
				...
				// insertion point sub template for init 
				// init the arrays
				frontRepo.array_Bars = []
				frontRepo.map_ID_Bar.clear()

				backRepoData.BarAPIs.forEach(
					barAPI => {
						let bar = new Bar
						frontRepo.array_Bars.push(bar)
						frontRepo.map_ID_Bar.set(barAPI.ID, bar)
					}
				)
				...
				backRepoData.BarAPIs.forEach(
					barAPI => {
						let bar = frontRepo.map_ID_Bar.get(barAPI.ID)
						CopyBarAPIToBar(barAPI, bar!, frontRepo)
					}
				)
